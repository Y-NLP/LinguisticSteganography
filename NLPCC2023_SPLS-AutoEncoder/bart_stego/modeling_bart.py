...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.length = 0
        self.bpw = 0
        self.self_attention = SelfAttention(768, 12, 0.1)
    ...
    def set_code(self, code, beams=100):
        if len(self.code) % self.bpw == 0:
            self.code = code
        else:
            self.code = code
            for i in range(self.bpw - (len(self.code) % self.bpw)):
                self.code.append(0)
        self.index = [0] * beams
        self.start_index = 0
        self.num = 0
    ...
    def set_mask(self, device, index_arr=None):
        self.tt = [[] for i in range(2 ** self.bpw)]
        # print(self.tt)
        tmp = torch.arange(0, 21128, dtype=torch.long).to(device)
        # tmp = torch.arange(0,4,dtype=torch.long).to('cuda:0')
        for i in range(2 ** self.bpw):
            self.tt[i] = (tmp % 2 ** self.bpw) == torch.ones_like(tmp) * i
            self.tt[i] = self.tt[i].reshape(1, 1, -1).to(torch.float) * -1e20

        mask_tmp = torch.zeros_like(self.tt[0][0])
        self.mask = [[] for i in range(2 ** self.bpw)]
        for i in range(2 ** self.bpw):
            self.mask[i] = copy.deepcopy(mask_tmp)
            # print(self.mask[i])
    ...
    def forward(
        ...
        lm_logits[:, :, 511] = lm_logits[:, :, 511] - 3
        lm_logits[:, :, 8024] = lm_logits[:, :, 8024] - 3
        lm_logits[:, :, 172] = lm_logits[:, :, 172] - 3
        lm_logits[:, :, 185] = lm_logits[:, :, 185] - 3
        # cover_emb = torch.mean(outputs.encoder_last_hidden_state, dim=1)#bart encoding
        # cover_emb=torch.max(outputs.encoder_last_hidden_state, dim=1)[0]
        token_emb = self.model.get_input_embeddings().weight
        cover_emb = torch.mean(token_emb[self.text], dim=0).unsqueeze(dim=0)
        # cover_emb = torch.max(token_emb[self.text], dim=0)[0].unsqueeze(dim=0)

        cosine_similarity = torch.cosine_similarity(cover_emb, token_emb, dim=1)
        lm_logits[0] += self.theta * cosine_similarity
        logits, indices = lm_logits[:, -1, :].sort(descending=True)
        logits = logits[:, :2 ** self.bpw]
        indices = indices[:, :2 ** self.bpw]

        for i in range(lm_logits.shape[0]):
            if self.index[i] < len(self.code):
                log_probs = F.log_softmax(logits, dim=-1)
                probs = torch.exp(log_probs)
                probs_array = probs.detach().cpu().numpy()
                coding = HuffmanCoding()
                coding.make_heap_from_array(probs_array[i])
                coding.merge_nodes()
                root = coding.make_codes()
                while root.token is None:
                    if self.index[i] >= len(self.code) or self.code[self.index[i]] == 0:
                        root = root.left
                    else:
                        root = root.right
                    self.index[i] += 1
                selection = root.token
                lm_logits[i, :, indices[0][selection]] = 100
                self.bpw_total[i] = self.bpw_total[i] + 1
            else:
                if self.flag:
                    lm_logits[i, :, 10] = 1000
                    # logits, indices = lm_logits[:, -1, :].sort(descending=True)
                    # lm_logits[i, :, indices[0][2 ** self.bpw]] = 100
                    # print("***************")
                    # self.flag=False
